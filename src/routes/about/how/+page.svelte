...
<article class="prose">
  ...
  <p>The app takes every georeferenced record with no flagged geospatial issue, drops living specimens (something somebody planted) and records marked introduced or managed, and tests each against the native range from WCVP, discarding records outside it. On what remains it finds the densest cluster: records are binned at 1°, and the bin with most neighbours wins if it holds at least half the records or twice the runner-up; if no bin dominates at 1° the test repeats at 2° and 4°, and a range where nothing dominates even at 4° is called disjunct and gets no centre. Inside the winning cluster the densest 1° population is found again, and the centre is the actual record nearest its middle, so the point is somewhere the plant has been seen and never the average of two mountain ranges.</p>
  <p>Restricted-licence records (CC BY-NC) are used for this clustering and for nothing else: the centre is a derived number, and every in-range coordinate may inform it, but only openly licensed records are published on the map or in the data. Three agreeing records are enough for a narrow endemic; under a dozen the page says the evidence is thin.</p>
  ...
  <p>The wet season is the smallest set of months that carries 70% of the year's rain. Four months that do is a sharp season; eight or more is no season, and so is a year whose monthly mean temperature moves less than 4 °C. Under 120 mm a year rain is not what drives the plant, so the season is taken from temperature instead: the six coolest months, which on a fog coast are the foggy ones. A wet season cooler than the year's mean is a winter grower; warmer, a summer grower.</p>
  ...
  <p>The paragraph at the top of the cultivation section is condensed by rule from the sheet's rows, one sentence per card, in the sheet's own figures. It lists the rows each sentence came from. It is not written by a person or a model, and the page says so.</p>
