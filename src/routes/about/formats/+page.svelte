...
plants.csv           one row per plant, for a spreadsheet (not for import)`}</code></pre>
  ...
  <p>A sealed blob is one version byte (1), a 12-byte IV, then AES-GCM ciphertext with associated data <code>vaultId|kind</code>, where kind is <code>log</code> or <code>photo</code>, so a blob cannot be replayed into another vault or as another kind; a photo's data also carries its id (<code>vaultId|photo|photoId</code>), so it cannot be handed out under another photo's id. A log batch decrypts to <code>{`{ "v": 1, "device": "...", "changes": [...] }`}</code> and is stored at <code>vault/&lt;id&gt;/log/&lt;hlc&gt;-&lt;hash&gt;.bin</code> where <code>&lt;hlc&gt;</code> is the batch's last change and <code>&lt;hash&gt;</code> the first twelve hex digits of the sealed bytes' SHA-256, so the same batch pushed twice lands on the same key (the server compares the bytes and says so) and two batches that differ never share a name. A batch the device cannot open or apply is set aside by name and reported, and does not stop the ones behind it. Batches are handed out in the order they <em>arrived</em> at the server, not in key order: a device that edited offline and uploads after another device has already pulled past that time must still be found, so a client asks for everything since a server arrival time, re-reads a minute of overlap, and skips batches it already holds. The changes inside carry the HLCs that settle merges; arrival order only decides discovery. A photo decrypts to a 4-byte big-endian length, the full JPEG, then the thumbnail, at <code>vault/&lt;id&gt;/photo/&lt;id&gt;.bin</code>. Nothing is rewritten or deleted.</p>
  <p>The endpoints: <code>POST /api/sync/vault</code> {`{ id, token, create }`}; <code>GET /api/sync/log?vault=&amp;since=&lt;ms&gt;</code> (returns <code>{`{ batches: [{ key, at }], more }`}</code>); <code>POST /api/sync/log?vault=</code> with header <code>X-Batch</code>; <code>GET /api/sync/log/&lt;hlc&gt;?vault=</code>; <code>PUT|GET|HEAD /api/sync/photo/&lt;id&gt;?vault=</code>. All but creation take <code>Authorization: Bearer &lt;token&gt;</code>.</p>
