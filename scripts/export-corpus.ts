 ...
 *
 *   npm run export                              # static/s/v1 → cultifolio-corpus-<date>.zip
 ...
 * s/v1/<key>.json), writes one zip. Nothing is fetched.
 */
...
const idxDir = existsSync(`${dir}/s/v1/index.json`) ? `${dir}/s/v1` : dir;
if (!existsSync(`${idxDir}/index.json`)) {
  console.error(`No index.json under ${dir} (looked in ${dir}/s/v1 and ${dir}). Build a corpus first: npm run dossier -- names.txt --grid climate`);
  process.exit(1);
}
...
const sDir = existsSync(`${dir}/s/v1`) ? `${dir}/s/v1` : dir;
