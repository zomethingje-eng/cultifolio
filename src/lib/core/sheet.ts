...
export interface Year {
  ...
  /** Under 120 mm a year: the season was read from temperature, not rain. */
  fog: boolean;
...
export function growingYear(months: Month[], lat: number | null | undefined): Year | null {
  ...
  // Almost no rain: the year is set by fog and temperature, not by rain. Growth follows the cooler half.
...
export function cultivationSheet(input: SheetInput): { rows: Row[]; arch: ArchGuess | null; year: Year | null } {
  const rows: Row[] = [];
  const add = (card: string, k: string, s: string, why: string, hab = false) => rows.push({ card, k, s, why, hab });
  ...
  if (dlis.length) {
    ...
    add('Light', 'Light', `Conventionally ${arch.dli[0]} to ${arch.dli[1]} mol/m²/day for a ${arch.lab.toLowerCase()}, stated as a band because that is the resolution conventional advice has.`, `What a ${arch.lab.toLowerCase()} usually wants; no habitat radiation figure is available for this species.`);
  }
  ...
  if (m && year) {
    ...
    const rain =
      year.annualMm < 120
        ? `Its habitat gets ${RAIN(year.annualMm)} a year, ${dryMonths ? `${dryMonths} months of it with under 5 mm` : 'thinly spread'}: this is a plant that lives on fog, dew and the occasional storm, and a pot that stays wet is the one thing it has never met.`
        : year.annualMm < 400
    ...
    add('Water', 'Water', (arch ? arch.water + ' ' : '') + rain, `Monthly rainfall from CHELSA at the habitat centre${arch ? `; the watering posture is what a ${arch.lab.toLowerCase()} wants, inferred from ${guess!.why}` : ''}.`, true);

    const seasonWord = year.grow === 'winter' ? 'a winter grower' : year.grow === 'summer' ? 'a summer grower' : 'not strongly seasonal';
    const hemi = year.south ? 'southern' : 'northern';
    const where = year.south ? 'grown north of the equator' : 'grown south of the equator';
    add(
      'Its year',
      'Its year',
      year.grow === 'even'
        ? `In habitat the rain does not divide the year sharply${range8(m) ? `, but the temperature does: the cooler months, ${wetSpanCool(m)}, are usually when a plant from here grows in cultivation, resting through the summer heat` : ', so expect it to slow in the extremes rather than stop'}. Water when it is growing and ease off when it is not, and let the plant, not the calendar, say which.`
        : year.fog
          ? `Rain is not what drives its year: ${RAIN(year.annualMm)} in a normal one. It grows in the cooler, foggier months, ${wetSpan} in habitat${where ? ` (${shiftedSpan} ${where})` : ''}, and sits tight through the hot ones. Water lightly in that cool season and hardly at all outside it.`
          : `In habitat it is ${seasonWord}: the rain comes ${wetSpan}, ${year.grow === 'winter' ? 'the cool half of the ' + hemi + '-hemisphere year' : 'the warm half of the ' + hemi + '-hemisphere year'}. ${where.charAt(0).toUpperCase() + where.slice(1)}, that season falls ${shiftedSpan}; most plants keep habitat timing for a year or two and then drift to the local one. Rest it ${span([...Array(12).keys()].map((i) => i + 1).filter((mm) => !year.growMonths.includes(mm)))} in habitat time: dry, bright, moving air, and no feed.`,
      `Worked out from the shape of the rainfall and temperature curves at the habitat centre (CHELSA). A calendar says what is expected, not what your plant is doing.`,
      true
    );
  } else if (arch) {
    add('Water', 'Water', arch.water, `The posture for a ${arch.lab.toLowerCase()}, inferred from ${guess!.why}. No habitat rainfall figure is available for this species.`);
  }
  ...
  if (m || arch) {
    ...
    if (bits.length)
      add('Warmth and air', 'Temperature', bits.join(' '), `${ex ? 'NASA POWER daily minima 1981–2024, lapse-corrected to the habitat centre' : m ? 'CHELSA monthly means at the habitat centre' : 'Conventional practice'}${arch?.minC != null ? `, held up to ${T(arch.minC)} because a ${arch.lab.toLowerCase()} has no dormancy to meet the cold in` : ''}. Not a measured survival limit.`, !!(m || ex));
  }
  ...
  if (arch) {
    ...
    add(
      ...
      arch.rh != null
        ...
        : `Humidity is not worth chasing for this one. What it wants is air movement and a mix that drains.${habRh != null ? ` Habitat averages ${habRh}% relative humidity${habRh >= 60 ? ', much of it as coastal fog the plant drinks through the night; that is not the same as a wet pot.' : '.'}` : ''}`,
      `What a ${arch.lab.toLowerCase()} wants${habRh != null ? '; the habitat figure is CHELSA relative humidity at the centre' : ''}.`,
      habRh != null
    );
    if (arch.waterQ !== 'any') add('Water', 'What you water it with', `${WATER_Q_LAB[arch.waterQ]}. ${WATER_Q_NOTE[arch.waterQ]}`, 'A requirement of the group, not of this species specifically.');
  }
